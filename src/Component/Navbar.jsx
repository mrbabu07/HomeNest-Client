...
// src/Component/Navbar.jsx - Enhanced Version
import React, { useState, useContext, useEffect } from "react";
  ...
  useEffect(() => {
    if (searchQuery.length > 2) {
      const timer = setTimeout(async () => {
        try {
          const response = await axios.get(
            `http://localhost:3000/allServices?search=${searchQuery}&limit=5`
          );
                  ...
                  <div className="absolute right-0 mt-2 w-80 bg-base-100 rounded-xl shadow-2xl border border-base-200 z-20 max-h-96 overflow-hidden flex flex-col">
                    ...
                    <div className="overflow-y-auto flex-1">
                      {notificationsLoading ? (
                        ...
                        <div className="p-8 text-center">
                          <Bell size={48} className="mx-auto text-base-content/30 mb-3" />
                          <p className="text-base-content/70 text-sm">No notifications yet</p>
                        </div>
                      ) : (
                        notifications.map((notification) => (
                          <div
            ...
            <div className="p-4 space-y-2">
              ...
              {user && (
                <>
                  ...
                  <NavLink to="/favorites" className={navLinkStyle} onClick={() => setMobileMenuOpen(false)}>
                    <FaHeart size={18} /> Favorites
                  </NavLink>
